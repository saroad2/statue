...
class AsynchronousEvaluationRunner(EvaluationRunner):
    ...
    async def evaluate_commands_map(self, commands_map, update_func):
        """
    ...
    async def evaluate_source(
        ...
        evaluation: Evaluation,
        update_func: Optional[Callable[[Evaluation], None]],
    ):
        """
        ...
        start_time = time.time()
        coros = [
            self.evaluate_command(command, source, evaluation, update_func)
            for command in commands
        ]
        await asyncio.gather(*coros)
        end_time = time.time()
