...
async def mock_repository_total_lock():
    with mock.patch.object(SourcesLocksRepository, "total_lock") as total_lock_patch:
...
async def test_get_source_lock_twice(tmp_path, mock_repository_total_lock):
    ...
    assert mock_repository_total_lock.acquire.await_args_list == [
        mock.call(),
        mock.call(),
    ]
    assert mock_repository_total_lock.release.call_count == 2
    assert mock_repository_total_lock.release.call_args_list == [
        mock.call(),
        mock.call(),
    ]
