...
def test_read_settings_with_multiple_commands(full_settings_with_boolean_contexts):
    commands = read_commands(full_settings_with_boolean_contexts)
    assert commands == [
...
def test_read_settings_with_non_passing_context(full_settings_with_boolean_contexts):
    commands = read_commands(
        full_settings_with_boolean_contexts, contexts=[NOT_EXISTING_CONTEXT]
    )
    assert commands == []


def test_read_settings_with_one_passing_context(full_settings_with_boolean_contexts):
    commands = read_commands(full_settings_with_boolean_contexts, contexts=[CONTEXT3])
...
def test_read_settings_with_two_passing_context(full_settings_with_boolean_contexts):
    commands = read_commands(full_settings_with_boolean_contexts, contexts=[CONTEXT2])
    assert commands == [
        Command(name=COMMAND1, help=HELP_STRING1, args=[ARG1, ARG2]),
        Command(name=COMMAND2, help=HELP_STRING2, args=[ARG3]),
    ]
...
def test_read_settings_with_two_contexts(full_settings_with_boolean_contexts):
    commands = read_commands(
        full_settings_with_boolean_contexts, contexts=[CONTEXT1, CONTEXT2]
    )
    assert commands == [
        Command(name=COMMAND1, help=HELP_STRING1, args=[ARG1, ARG2]),
    ]
...
def test_read_settings_with_non_standard_command(full_settings_with_boolean_contexts):
    commands = read_commands(full_settings_with_boolean_contexts, contexts=[CONTEXT4])
    assert commands == [
...
def test_read_settings_with_overrides_without_contexts(
    ...
    commands = read_commands(full_settings_with_override_contexts)
    assert commands == [
        Command(name=COMMAND1, help=HELP_STRING1, args=[ARG1, ARG2]),
    ]
...
def test_read_settings_with_overrides_with_context(
    ...
    commands = read_commands(full_settings_with_override_contexts, contexts=[CONTEXT1])
    assert commands == [
...
def test_read_settings_with_overrides_with_another_context(
    ...
    commands = read_commands(full_settings_with_override_contexts, contexts=[CONTEXT2])
    assert commands == [
...
def test_read_settings_with_overrides_with_clear_args_context(
    ...
    commands = read_commands(full_settings_with_override_contexts, contexts=[CONTEXT3])
    assert commands == [
...
def test_read_settings_with_overrides_with_add_args_context(
    ...
    commands = read_commands(full_settings_with_override_contexts, contexts=[CONTEXT4])
    assert commands == [
...
def test_read_commands_with_deny_list(full_settings_with_boolean_contexts):
    commands = read_commands(
        full_settings_with_boolean_contexts, deny_list=[COMMAND1, COMMAND3]
    )
    assert commands == [
        Command(name=COMMAND2, help=HELP_STRING2, args=[ARG3],),
        Command(name=COMMAND4, help=HELP_STRING4, args=[ARG4, ARG5]),
